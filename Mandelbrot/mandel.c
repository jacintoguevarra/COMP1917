...
void serveBMP (int socket, float centrex, float centrey, float z) {
  ...
  while( col < 512){
       bmp[3*i] = escapeSteps(x,y);
       //stepsToRed(escapeSteps(x,y));
     bmp[3*i+1] = escapeSteps(x,y);
     //stepsToBlue(escapeSteps(x,y));
     bmp[3*i+2] = escapeSteps(x,y);
     //stepsToGreen(escapeSteps(x,y));
        i ++;
        x += zoom;      
       //zoom;            
        col ++;
